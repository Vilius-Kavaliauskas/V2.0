...
/// Duomenu nuskaityma atliekanti funkcija.
///
/// Pagal nurodyta indeksa kiek_stud (3 <= kiek_stud <= 7) surandamas ir nuskaitomas failas su studentu suvestine.
void input_class(vector <Studentas>& stu, int kiek_stud)
{
    ...
    IN.close();
    cout << fixed << setprecision(3) << "CLASS : Failas nuskaitytas per " << skaito_vec.elapsed() << " s." << endl;
}